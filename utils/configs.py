...
def get_configs(args=None) -> ExpConfigs:
    ...
    parser.add_argument('--lr_scheduler', type=str, choices=["ExponentialDecayLR", "ManualMilestonesLR", "DelayedStepDecayLR", "CosineAnnealingLR", "MultiStepLR", "OneCycleLR", "StepLR"], default='DelayedStepDecayLR', help='learning rate scheduler. Originally named as --lradj')
    parser.add_argument('--lr_scheduler_gamma', type=float, default=0.1, help='gamma for StepLR and MultiStepLR.')
    parser.add_argument('--gate_warmup_epochs', type=int, default=10, help='epochs used to transition from global attention to predicted gates')
    parser.add_argument('--max_lr', type=float, default=None, help='maximum learning rate for OneCycleLR')
    parser.add_argument('--pct_start', type=float, default=0.3, help='fraction of OneCycleLR steps spent increasing the learning rate')
    ...
    parser.add_argument('--train_epochs', '--epochs', dest='train_epochs', type=int, default=300, help='train epochs; --epochs is an alias for OneCycleLR')
